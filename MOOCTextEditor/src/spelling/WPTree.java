...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode src = new WPTreeNode(word1,null);
		queue.add(src);
		List<String> ret = new LinkedList<>();
		WPTreeNode des = null;
		Map<String,WPTreeNode> found = new HashMap<>();
		found.put(word1, src);
		while(!queue.isEmpty()){
			WPTreeNode s = queue.poll();
			//System.out.println(s.getWord());
			if(s.getWord().compareTo(word2)==0){
				des = s;
				break;
			}
			List<String> l = nw.distanceOne(s.getWord(), true);
			for(String a : l){
				if(!found.containsKey(a)){
					WPTreeNode node = new WPTreeNode(a, s);
					s.addChild(a);
					found.put(a, node);
					queue.add(node);
				}
