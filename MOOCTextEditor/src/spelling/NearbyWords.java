...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i=0;i<=s.length();i++){
			for(char c = 'a' ;c<='z';c++){
				String word ="";
				if(i!=0)word = s.substring(0, i);
				word += c;
				word = word.concat(s.substring(i));
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i=0;i<s.length();i++){
			String word = "";
			if(i!=0)word = s.substring(0, i) + s.substring(i+1);
			else word = s.substring(i+1);
			if(!currentList.contains(word) && 
					(!wordsOnly||dict.isWord(word)) &&
					!s.equals(word)) {
				currentList.add(word);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty()){
			...
			List<String> l = distanceOne(s, true);
			for(String st : l){
